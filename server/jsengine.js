...
/*
	...
	set(indiv,property_name,value){	     
		...
		// TODO desactivado inmutable aqui por siguiente linea: let
		// mutind=Immutable.Seq(indiv).toObject();
		// TODO para desactivar inmutable es necesario hacer un set a individual
		// store y actualizar arrInverse
		let mutind=indiv;
		// FIN desactivacion
	...
	log_property_change_byindex(index_name,individual,property,action){
		...
		for(let rule_data of rule_list){
			// TODO es poco eficiente por cada cambio recorrer todas las reglas,
			// deberia tener un mapa de propiedades dependientes hacia su regla
	...
	new_individual(indiv){
		...
		// al crearse individuos en JSE, se registran en log, y mas tarde se
		// chequea su indexacion. En cargas masivas propagate esta inhabilitado
		// hasta el final
		this.propagate();
	}

	delete_object(indiv){
		// TODO antes de actualizar los indices, debe propagarse la accion del,
		// así los indices todavia saben cuando les afecta
	}
	...
	propagate_index(){
		...
		this.log_index=[];
		// al propagarse los indices, puede modificarse otros indices y
		// registrarse en este log
	...
	propagate_rules(){
		// TODO Una regla menos prioritaria puede ser que cuando se vaya a
		// disparar no se cumple condiciones, debido a una regla previa
